...
public class ScriptEngineTest {
	...
	public void testE4XHelloWorld() {
		...
		try {
			...
			eEngine.eval("var d = new XML('<d><item>Hello</item><item>World!</item></d>');print('E4X - ' + d..item);");
		} catch (Exception ex) {
			ex.printStackTrace();
			assertFalse(true);
		}
	...
	/*
	 * @Test public void testEngines() { Map<String, ScriptEngineFactory>
	 * engineFactories = new HashMap<String, ScriptEngineFactory>(7);
	 * ScriptEngineFactory[] factories = mgr.getEngineFactories(); for
	 * (ScriptEngineFactory factory : factories) {
	 * System.out.println("\n--------------------------------------------------------------");
	 * String engName = factory.getEngineName(); String engVersion =
	 * factory.getEngineVersion(); String langName = factory.getLanguageName();
	 * String langVersion = factory.getLanguageVersion();
	 * System.out.printf("Script Engine: %s (%s) Language: %s (%s)", engName,
	 * engVersion, langName, langVersion); engineFactories.put(engName,
	 * factory); String[] engNames = factory.getNames();
	 * System.out.print("\nEngine Alias(es):"); for (String name : engNames) {
	 * System.out.printf("%s ", name); } String[] ext = factory.getExtensions();
	 * System.out.printf("\nExtension: "); for (String name : ext) {
